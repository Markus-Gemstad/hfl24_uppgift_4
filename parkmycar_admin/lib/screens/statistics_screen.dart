...
class _StatisticsScreenState extends State<StatisticsScreen> {
  ...
  Future<List<Parking>> getOngoingParkings() async {
    ...
    var items = await ParkingHttpRepository()
    ...
    for (var item in items) {
      ...
      try {
        item.parkingSpace =
            await ParkingSpaceHttpRepository().getById(item.parkingSpaceId);
      } catch (e) {
  ...
  Future<List<PopularParkingSpace>> getPopularParkingSpaces() async {
    var parkingSpaces = await ParkingSpaceHttpRepository().getAll();
    var parkings = await ParkingHttpRepository().getAll();
  ...
  Future<double> getTotalParkingIncome() async {
    var items = await ParkingHttpRepository().getAll();
    items = items.where((element) => !element.isOngoing).toList();
